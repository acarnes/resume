...
\hspace{20 mm}
    ...
    \begin{stuff}
        \vspace*{1mm}
                \item Developed an NLP algorithm named SCUBA which reconciles supply chain spend data against written contracts to find unclaimed rebates
                \item Implemented SCUBA in partnership with supply chain leadership to save over \$1 million per year for continental division
                \item Created a machine learning algorithm called OSPRI that flags patients at high risk for pressure injuries (bed sores) 
                \item Implemented OSPRI in cooperation with the wound care nurses at Swedish hospital to reduce reportable pressure injuries by 60\%
                \item Created a machine learning algorithm named AMISS that detects present on admission septic shock 1.5 hours faster than the average emergency department (ED) doctor
                \item Implemented AMISS to flag septic patients and alert ED physicians by text message in real time
                \item Modeled the spread of COVID-19 for continental division hospitals to determine if our ICU capacity would be overwhelmed 
                %\item Developed AI computing infrastructure with fast GPUs and solid state storage to facilitate imaging and NLP AI on large datasets
                %\item Created the partnerships and codebase to query all radiologist reports and images for the division through all time
                %\item Created a dataset of 1.7 million chest xrays and radiologist reports for use with image recognition tasks
                %\item Trained a convolutional neural network to identify pneumothorax (collapsed lungs) in chest xrays better than any algorithm in literature
    \end{stuff}
    ...
    \begin{stuff}
        \vspace*{1mm}
                ...
                \item Automatic Categorization to Maximize Sensitivity of New Physics Searches based on Machine Learning -- Under peer review 
    \end{stuff}
    ...
    {\bf Miscellaneous:} Machine Learning Development in C++ and Python, \\ 
                         ...
                         Object Oriented Programming, git, Docker
    \end{tabular}
